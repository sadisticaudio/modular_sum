...
def modular_sum(a,b,freqs, *, out_mags=None, out_phases=None, inout_norms=(3.19, 0.101), get_tensors=False):
    ...
    # n_phases = torch.arange(d_model) * 2 * np.pi / d_model - np.pi
    # n_phases2 = ((n_phases+np.pi) * 2) - np.pi
    harmonic1 = torch.cos(prange * freqs[...,None,None] * 2 * np.pi / p + phzrange[...,None] - np.pi)
    harmonic2 = torch.cos(prange * 2 * freqs[...,None,None] * 2 * np.pi / p + (phzrange * 2)[...,None] - np.pi)
    ...
    weights = (torch.cos(out_phases - (phzrange[...,None,None] - 0 * np.pi)) * out_mags).permute(2,0,1)
